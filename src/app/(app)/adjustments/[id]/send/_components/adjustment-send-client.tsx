...
export function AdjustmentSendClient({
  adjustmentId,
  quoteId,
  initialTo,
  initialCc,
  ...
  async function handleSend() {
    ...
    setSending(true);
    try {
      const res = await fetch(`/api/adjustments/${adjustmentId}/send`, {
        method: 'POST',
      ...
      toast.success(
        isTestSend ? `테스트 메일 발송 완료 → ${loggedInUserEmail ?? '본인'}` : '조정 안내 메일 발송 완료',
      );
      ...
      <Card>
        <CardContent className="p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-gray-900">조정 안내 메일</h2>
            <Button variant="ghost" size="sm" asChild>
              <Link href={`/quotes/${quoteId}`}>
                <ArrowLeft className="h-4 w-4 mr-1" /> 견적으로 이동
              </Link>
            </Button>
          </div>
          ...
          <div>
            <Label className="text-xs">
              받는사람 (To){' '}
              {isTestSend && (
                <span className="text-blue-600">— 테스트 모드: 본인에게만 발송됩니다</span>
              )}
            </Label>
            <Textarea
              rows={2}
      ...
      <Card className="border-blue-200 bg-blue-50/50">
        <CardContent className="p-6">
          ...
          <p className="mt-3 text-xs text-gray-600">
            ℹ 조정 안내 메일은 견적 상태를 변경하지 않습니다 — 이력만 추가됩니다.
          </p>
        </CardContent>
      </Card>
    </div>
