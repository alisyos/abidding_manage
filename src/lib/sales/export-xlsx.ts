 ...
 *  R1: 거래처 / 세부거래처 / 견적번호 / 상태 / [매체 그룹: K, S, M] (각 4셀 merge) / 기본가액 / 변동조정 / 합계 / 입금일
 *  R2: (앞 4컬럼 비움) / 유니크/프리미엄/베이직/라이트 × 3매체 / (뒤 4컬럼 비움)
 *  R3~: 데이터
 *  마지막: 합계 행
 */
export function buildSalesWorkbook(pivot: PivotResult, month: string): ArrayBuffer {
  ...
  for (const row of pivot.rows) {
    ...
    r.push(row.base_amount, row.variable_adjust, row.total_amount, row.payment_date ?? '');
    aoa.push(r);
  }
